...
/**
 * Catalogue product card (used on /products, search and the similar-products
 * rows in every skin). Inner class names are all `sr-`-prefixed on purpose:
 * generic names (.cat/.info/.name…) collided with the Brand skin's own
 * `.brand-root .cat` category-tile rules and blew the card layout apart.
 */
export function ShowroomCard({
  ...
  return (
    ...
    >
      ...
      />
      <div className="sr-imgbox">
        <span className="sr-btag">{product.brandName}</span>
        <Image
      ...
      <div className="sr-body">
        <span className="sr-kick">{product.categoryName}</span>
        <h3 className="sr-name">{product.name}</h3>
        <p className="sr-desc" dir="ltr">{product.cardSpec}</p>
        <div className="sr-meta">
          <Stars value={rating.avg} count={rating.count} />
          <span className="sr-stock">{tCat('inStock')}</span>
        </div>
