...
/* ROUND 19 — one glyph per catalogue family, drawn from the same 24×24
   stroke vocabulary as EDPATH so the rail, the mega-menu and the family
   headers stay visually of a piece. */
const FAM_PATH: Record<string, string> = {
  work: EDPATH.laptop ?? '',
  display: EDPATH.desktop ?? '',
  print: EDPATH.print ?? '',
  build: EDPATH.parts ?? '',
  net: EDPATH.network ?? '',
  power: EDPATH.bolt ?? '',
...
export function FamilyIcon({ n, s = 18, sw = 1.7 }: { n: string; s?: number; sw?: number }) {
  const d = FAM_PATH[n] ?? FAM_PATH.other ?? ''
  return (
