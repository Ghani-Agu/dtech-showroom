...
/* Chaque bloc visuel est exporté à part pour que l'éditeur web puisse les
   réordonner ou en masquer un individuellement ; ils recalculent tous ce dont
   ils ont besoin (`useEditorial()`, `brandFacts()`…) au lieu de le recevoir. */

/**
 * Les props de la racine `div.edb`, en un seul endroit.
 *
 ...
 * Le `data-status` et les `--bc`/`--bfg` restent donc sur la racine, jamais
 * sur les sections : sorties d'ici, elles perdraient leur couleur.
 */
export function edBrandRootProps(data: EdBrandPageData): {
  className: string
  ...
  return { className: 'edb', style: accent as React.CSSProperties, 'data-status': facts.status }
}

/** ── Hero: a full-bleed band in the brand's own colour ── */
export function EdBrHero({ data }: { data: EdBrandPageData }) {
...
export function EdBrStatBand({ data }: { data: EdBrandPageData }) {
  const { t, tf } = useEditorial()
  const B = data.name
...
/** ── Description + why ── */
export function EdBrWhy({ data }: { data: EdBrandPageData }) {
  const { t, tf } = useEditorial()
  const facts = brandFacts(data.slug)
  const B = data.name
...
export function EdBrCats({ data }: { data: EdBrandPageData }) {
  const { t, tf } = useEditorial()
  const B = data.name

  if (data.cats.length === 0) return null
...
export function EdBrSelection({ data }: { data: EdBrandPageData }) {
  const { t, tf } = useEditorial()
  const B = data.name
...
/** ── FAQ ── */
export function EdBrFaq({ data }: { data: EdBrandPageData }) {
  const { t, tf } = useEditorial()
  const B = data.name
...
export function EdBrCta({ data }: { data: EdBrandPageData }) {
  const { t, tf } = useEditorial()
  const B = data.name
...
/** ── Sibling brands ── */
export function EdBrOthers({ data }: { data: EdBrandPageData }) {
...
export function EdBrandPage({ data }: { data: EdBrandPageData }) {
  return (
    <div {...edBrandRootProps(data)}>
      <EdBrHero data={data} />
      <EdBrStatBand data={data} />
      <EdBrWhy data={data} />
      <EdBrCats data={data} />
      <EdBrSelection data={data} />
      <EdBrFaq data={data} />
      <EdBrCta data={data} />
      <EdBrOthers data={data} />
    </div>
  )
}
