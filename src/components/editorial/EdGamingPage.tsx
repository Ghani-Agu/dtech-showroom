 ...
 *
 * Chaque section visuelle est EXPORTÉE séparément (`EdGm*`), avec la même
 * signature `{ data }`, pour que l’éditeur web puisse les réordonner ou en
 * masquer une sans toucher au code. `EdGamingPage` ne fait plus que les
 * composer dans l’ordre d’origine ; l’état et les effets voyagent avec leur
 * section (le carrousel du héros, l’onglet des collections).
 */
...
export function EdGmAurora(_: { data: EdGamingData }) {
  return (
...
/* ── Hero ── */
export function EdGmHero({ data }: { data: EdGamingData }) {
...
export function EdGmRgbBar(_: { data: EdGamingData }) {
  return (
    <div className="wrap">
...
/* ── Brand ticker ── */
export function EdGmBrandTicker({ data }: { data: EdGamingData }) {
...
/* ── Build path ── */
export function EdGmBuildPath({ data }: { data: EdGamingData }) {
  const { t } = useEditorial()
...
export function EdGmCollections({ data }: { data: EdGamingData }) {
...
/* ── Why here ── */
export function EdGmWhy(_: { data: EdGamingData }) {
  const { t } = useEditorial()
...
/* ── CTA ── */
export function EdGmCta(_: { data: EdGamingData }) {
  const { t } = useEditorial()
...
export function EdGamingPage({ data }: { data: EdGamingData }) {
  return (
    <div className="edg">
      <EdGmAurora data={data} />
      <EdGmHero data={data} />
      <EdGmRgbBar data={data} />
      <EdGmBrandTicker data={data} />
      <EdGmBuildPath data={data} />
      <EdGmCollections data={data} />
      <EdGmWhy data={data} />
      <EdGmCta data={data} />
    </div>
  )
